...
KLect.send = function (lab, researcher, experiment, email, path) {
    ...
    $.ajax({
        type: 'POST',
        url: `${window.location.hostname}/${path}/KLect/KLect.php`,
...
KLect.fail = function() {
    ...
    let fail_txt = `Uh oh, something has gone wrong delivering your data to the server. Please copy everything below and send it to the following e-mail address:<br /> <a href='mailto: ${email}'>${email}</a>:`;
